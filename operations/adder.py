...
def add_matrices(matrix1, matrix2):
	...
	result_data = [[matresult_data = [[0 for _ in range(matrix1.cols)] for _ in range(matrix1.rows)]
	for i in range(matrix1.rows):
		for j in range(matrix1.cols):
			result_data[i][j] = matrix1.data[i][j] + matrix2.data[i][j]rix1.data[i][j] + matrix2.data[i][j] for j in range(matrix1.cols)] for i in range(matrix1.rows)]
